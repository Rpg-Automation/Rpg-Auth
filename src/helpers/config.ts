...
const config = {
	...
	CLIENT_CALLBACK_URL: <string>process.env.CLIENT_CALLBACK_URL || undefined,
...
if (config.CLIENT_ID === undefined || config.CLIENT_SECRET === undefined || config.CLIENT_CALLBACK_URL === undefined || config.DASHBOARD_CALLBACK_URL === undefined) {
	throw Error("CLIENT_ID || CLIENT_SECRET || CLIENT_CALLBACK_URL not specified || DASHBOARD_CALLBACK_URL not specified");
}
